...
export default function EmpresaPresupuestosPage() {
  ...
  const fetchPresupuestos = async () => {
    try {
      setLoading(true);
      const params = [];
      if (filtroEstado) params.push(`status=${filtroEstado}`);
      if (busqueda) params.push(`search=${busqueda}`);
      const queryString = params.length > 0 ? `?${params.join('&')}` : '';

      const response = await fetch(`/api/empresa/presupuestos${queryString}`);
      ...
      <div className="mb-6">
        <div>
          ...
          <p className="text-gray-600">
            Consulta i gestiona els pressupostos rebuts
          </p>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-6">
          ...
          <div className="bg-blue-50 rounded-lg shadow p-4">
            <div className="text-2xl font-bold text-blue-900">{stats.pendientes}</div>
            <div className="text-sm text-blue-700">Pendents</div>
          </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1">
            <input
              ...
              placeholder="Buscar per número o concepte..."
        ...
        <div className="bg-white rounded-lg shadow p-12 text-center">
          ...
          <p className="text-gray-600 mb-4">
            {filtroEstado
              ...
              : 'Encara no has rebut cap pressupost'}
          </p>
          <Link
            href="/empresa/extras"
            ...
            >
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mb-1">
                      Creat per: <span className="font-medium">{presupuesto.creador.name}</span>
                    </p>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm text-gray-500">
                      Creat: {formatDate(presupuesto.fechaCreacion)}
                    </div>
                    {presupuesto.fechaEnvio && (
                      <div className="text-sm text-gray-500">
                        Enviat: {formatDate(presupuesto.fechaEnvio)}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-4 border-t border-gray-200">
                  <Link
                    href={`/empresa/presupuestos/${presupuesto.id}`}
                  ...
                  {presupuesto.estado === 'ENVIADO' && (
                    <>
                      <button
                        onClick={() => window.location.href = `/empresa/presupuestos/${presupuesto.id}?action=accept`}
                        className="px-4 py-2 bg-green-600 text-white rounded-md text-sm font-medium hover:bg-green-700"
                      >
                        Acceptar
                      </button>
                      <button
                        onClick={() => window.location.href = `/empresa/presupuestos/${presupuesto.id}?action=reject`}
