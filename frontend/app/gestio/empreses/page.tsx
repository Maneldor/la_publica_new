...
import { getOrCreateConversation, getConversationWith } from '@/lib/messaging/actions'

export default function EmpresesGestioPage() {
  const router = useRouter()
  const { data: session } = useSession()

  // State
  const [empreses, setEmpreses] = useState<EmpresaLlista[]>([])
  const [stats, setStats] = useState<EmpresaStats | null>(null)
  const [gestors, setGestors] = useState<{ id: string; name: string; email: string }[]>([])
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Usar fetch directo para debug
      const [empresesRes, statsRes, gestorsRes] = await Promise.all([
        fetch('/api/gestio/empreses?' + new URLSearchParams(
          Object.entries(filters).filter(([_, v]) => v !== undefined).map(([k, v]) => [k, Array.isArray(v) ? v.join(',') : v as string])
        )),
        fetch('/api/gestio/empreses/stats'),
        hasFullAccess ? fetch('/api/gestio/gestors') : Promise.resolve({ ok: true, json: () => ({ data: [] }) })
      ])

      const empresesResult = empresesRes.ok ? await empresesRes.json() : { error: 'Error HTTP: ' + empresesRes.status, data: [] }
      const statsResult = statsRes.ok ? await statsRes.json() : { error: 'Error HTTP: ' + statsRes.status, data: { total: 0, verificades: 0, actives: 0, pendents: 0, pendentsCompletar: 0 } }
      const gestorsResult = gestorsRes.ok ? await gestorsRes.json() : { error: 'Error HTTP: ' + gestorsRes.status, data: [] }

      // Siempre cargar los datos, mostrando errores solo si es necesario
      setEmpreses(empresesResult.data || [])
      setStats(statsResult.data || { total: 0, verificades: 0, actives: 0, pendents: 0, pendentsCompletar: 0 })
      setGestors(gestorsResult.data || [])

      // Mostrar errores solo si hay datos importante que faltan
      if (empresesResult.error && !empresesResult.data?.length) {
        toast.error('Error carregant empreses: ' + empresesResult.error)
      }

      if (statsResult.error && !statsResult.data) {
        toast.error('Error carregant estadístiques: ' + statsResult.error)
      }
    } catch (error) {
  ...
  }, [session, searchTerm, selectedStatus, selectedPlan, selectedGestor])
  ...
  const handleOpenMissatgeria = async (empresa: EmpresaLlista) => {
    try {
      // Per simplicitat, navegar directament a missatgeria amb la companyId
      // Les server actions gestionaran la cerca del propietari
