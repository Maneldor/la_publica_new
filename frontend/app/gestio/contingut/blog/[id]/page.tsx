...
export default function EditBlogPostPage() {
  ...
  const loadPost = async () => {
    try {
      ...
      if (res.ok && data.post) {
        ...
        setSelectedGroups(post.targetGroups.map(g => g.id))
        ...
        if (post.poll) {
          setHasPoll(true)
          setPollId(post.poll.id)
          setPollQuestion(post.poll.question)
          setPollType(post.poll.type)
          setPollOptions(post.poll.options.map(o => o.text))
          setPollShowResults(post.poll.showResults)
          setPollEndsAt(post.poll.endsAt ? new Date(post.poll.endsAt).toISOString().slice(0, 16) : '')
        }
      } else {
        router.push('/gestio/contingut/blog')
      }
    } catch (error) {
      console.error('Error carregant article:', error)
      router.push('/gestio/contingut/blog')
    } finally {
      setIsFetching(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (!hasPoll && pollId) {
        // Remove poll if was disabled
        postData.removePoll = true
      }
      ...
      if (res.ok) {
        ...
        alert(error.error || 'Error actualitzant article')
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (!confirm('Estàs segur que vols eliminar aquest article? Aquesta acció no es pot desfer.')) {
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const error = await res.json()
        alert(error.error || 'Error eliminant article')
      }
    } catch (error) {
      console.error('Error:', error)
      alert('Error de connexió')
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Editar article</h1>
            <p className="text-gray-600">Modifica l&apos;article existent</p>
          </div>
        </div>
        <button
          type="button"
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              >
                <p className="font-medium text-gray-900">Publicar</p>
              </button>
              <button
                type="button"
          ...
          <div className="flex items-center justify-between">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
              />
              <span className="text-sm text-gray-900">{pollId ? 'Mantenir enquesta' : 'Afegir enquesta'}</span>
            </label>
          </div>
