...
export default function GestioEmpresesPendentsPage() {
  ...
  // Verificar permisos - Només ADMIN i SUPER_ADMIN
  useEffect(() => {
  ...
  // IMPORTANT: Canviar la ruta per crear empresa dins de /gestio/ o usar modal
  const handleCrearEmpresa = (lead: LeadPendentRegistre) => {
    // Opció 1: Redirigir a la pàgina existent d'admin (temporal)
    router.push(`/gestio/admin/empreses/crear?leadId=${lead.id}`)

    // Opció 2: Futur - Modal dins de /gestio/
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm text-slate-500">
        ...
        <span>/</span>
        <span className="text-slate-700">Empreses Pendents</span>
      </div>
...
function LeadDetailPanel({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-5 sticky top-6 space-y-5">
      ...
      {/* Enllaç al Pipeline */}
      <Link
        href={`/gestio/leads/pipeline?highlight=${lead.id}`}
