    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          console.log('🔍 Debug empresa creada:', { leadId, companyId, result })

          if (leadId && companyId) {
            console.log('✅ Cridant marcarLeadGuanyat amb:', { leadId, companyId })
            try {
              await marcarLeadGuanyat(leadId, companyId);
              console.log('✅ marcarLeadGuanyat completat')
            } catch (error) {
              console.error('❌ Error marcant lead com guanyat:', error);
            }
          } else {
            console.log('❌ No es pot marcar lead com guanyat:', { leadId, companyId })
          }
          ...
          // CANVI IMPORTANT: Redirigir a /gestio/admin/empreses-pendents en lloc de /admin/empresas/listar
          router.push('/gestio/admin/empreses-pendents');
        } else {
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="flex justify-between mt-8 pt-6 border-t">
            ...
            <div className="flex gap-4">
              <button
                onClick={() => router.push('/gestio/admin/empreses-pendents')}
                className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors"
              >
                Cancel·lar
              </button>
