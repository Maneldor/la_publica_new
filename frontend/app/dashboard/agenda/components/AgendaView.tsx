...
export function AgendaView({ userId, userName }: AgendaViewProps) {
  ...
  const [eventForm, setEventForm] = useState({ time: '', title: '', description: '' })
  
  // Estados para editar hábitos
  const [editingHabit, setEditingHabit] = useState<string | null>(null)
  const [editingHabitName, setEditingHabitName] = useState('')

  // Hook que maneja toda la lógica de datos
  const {
    isLoading,
    config,
    esdeveniments,
  ...
  const handleOpenEventModal = (event?: Esdeveniment) => {
    if (event) {
      ...
      setEventForm({ time: '', title: '', description: '' })
    }
                ...
                {esdeveniments.length > 0 ? (
                  <div className="space-y-2">
                    {esdeveniments.map((event) => (
                      ...
                      >
                        <span className="text-sm font-medium text-blue-600 w-14">{event.time}</span>
                        <div className="flex-1">
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-end gap-3 mt-6">
                ...
                <button
                  onClick={async () => {
                    if (!eventForm.time || !eventForm.title.trim()) return
                  ...
                  disabled={!eventForm.time || !eventForm.title.trim()}
