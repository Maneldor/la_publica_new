    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">Anunci destacat:</span>
              <span className={`font-medium ${formData.isFeatured ? 'text-yellow-600' : 'text-gray-500'}`}>
                {formData.isFeatured ? '⭐ Sí - Destacat' : '❌ No'}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span className={`font-medium ${
                ...
                formData.priority === 'high' ? 'text-orange-600' : 'text-gray-600'
              }`}>
                {formData.priority === 'urgent' ? '🚨 Urgent' :
                 formData.priority === 'high' ? '🔥 Alta' : '📄 Normal'}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Estat de moderació:</span>
              <span className={`font-medium ${formData.moderationStatus === 'approved' ? 'text-green-600' : 'text-yellow-600'}`}>
                {formData.moderationStatus === 'approved' ? '✅ Aprovat' : '⏳ Pendent'}
              </span>
            </div>
            {(formData.isFeatured || formData.priority !== 'normal') && (
              <div className="pt-3 border-t border-purple-200">
                <div className="bg-purple-100 rounded-lg p-3">
                  <p className="text-purple-800 text-xs font-medium">
                    🌟 Aquest anunci tindrà visibilitat especial gràcies als privilegis d'administrador
                  </p>
                </div>
              </div>
            )}
